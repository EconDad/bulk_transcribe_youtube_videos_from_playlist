#!/usr/bin/env python3
"""Resilient v4.3 diagnostic entrypoint for integration acceptance.
...
- repair calculation-array ordering without changing semantic item content;
- drop blank inventory array members and canonicalize generic arithmetic aliases;
...
- recover only source-grounded generic audit phrasing such as ``off the top``
  subtraction and ``percent of`` multiplication, including spoken number words;
- downgrade failed audit reconciliations only when their claimed operation is
  absent from the entire bounded neighborhood;
...
- localize paraphrased entailment quotes to exact text inside unchanged cited
  ranges before rerunning strict validation; and
- normalize punctuation-only entailment schema-key typos without dropping or
  inventing fields.
"""
...
from research_v43.inventory_recovery import AdaptiveInventoryOllamaClient
...
def main(argv=None) -> int:
    ...
    base.parse_inventory_response = parse_inventory_response_with_gate3_repair
    ...
    base.parse_inventory_evidence_audit_response = (
        parse_inventory_evidence_audit_response_with_gate3_repair
    )
